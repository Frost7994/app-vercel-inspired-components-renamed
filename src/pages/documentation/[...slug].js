...
const Documentation = ({ data, content, breadcrumbs, currentCrumb }) => {
  ...
  return (
    <>
      ...
      {/* mdx footer */}
      <MDXFooter previousCrumb={previousCrumb} nextCrumb={nextCrumb} />
    </>
  );
};
...
export const getStaticPaths = async () => {
  ...
  const documentation = folders.map((folder) => {
    return fs
      .readdirSync(path.join("src/data/documentation", folder))
      .map((file) => {
        // remove file extension and add folder to slug
        const slug = `/${folder}/${file.replace(".mdx", "")}`;

        const fileContents = fs.readFileSync(
          path.join("src/data/documentation", folder, file),
          "utf8"
        );
  ...
  const paths = documentation
    .flat()
    .map((doc) => ({ params: { slug: doc.slug.split("/").slice(1) } }));
...
export const getStaticProps = async ({ params }) => {
  ...
  // get file contents
  const fileContents = fs.readFileSync(
    path.join(`src/data/documentation/${folder}/${file}.mdx`),
    "utf8"
  );
